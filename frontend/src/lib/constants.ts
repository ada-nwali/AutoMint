...
/** The five deployed AutoMint contracts. */
export type ContractName = "registry" | "botNft" | "accrual" | "marketplace" | "token";

/** Contract IDs — the single source of truth, keyed by {@link ContractName}. */
export const CONTRACT_ADDRESSES: Readonly<Record<ContractName, string>> = {
  registry:
...
export function validateEnv(env: Record<string, string | undefined> = process.env): string[] {
  ...
  const ids: Array<[string, string | undefined]> = [
    ["NEXT_PUBLIC_REGISTRY_CONTRACT_ID", env.NEXT_PUBLIC_REGISTRY_CONTRACT_ID ?? CONTRACT_ADDRESSES.registry],
    ["NEXT_PUBLIC_BOT_NFT_CONTRACT_ID", env.NEXT_PUBLIC_BOT_NFT_CONTRACT_ID ?? CONTRACT_ADDRESSES.botNft],
    ["NEXT_PUBLIC_ACCRUAL_CONTRACT_ID", env.NEXT_PUBLIC_ACCRUAL_CONTRACT_ID ?? CONTRACT_ADDRESSES.accrual],
    ["NEXT_PUBLIC_MARKETPLACE_CONTRACT_ID", env.NEXT_PUBLIC_MARKETPLACE_CONTRACT_ID ?? CONTRACT_ADDRESSES.marketplace],
    ["NEXT_PUBLIC_TOKEN_CONTRACT_ID", env.NEXT_PUBLIC_TOKEN_CONTRACT_ID ?? CONTRACT_ADDRESSES.token],
  ];
